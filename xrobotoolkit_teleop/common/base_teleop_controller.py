...
class BaseTeleopController(abc.ABC):
    def __init__(
        ...
        self._grip_threshold = float(os.environ.get("XROBO_GRIP_THRESHOLD", "0.5"))
        self._debug_xr = os.environ.get("XROBO_DEBUG", "").lower() in ("1", "true", "yes")
        self._last_debug_t = 0.0
        self._grip_hint_shown = False
    ...
    def _handle_data_collector_grip(self, src_name: str) -> None:
        """Start or stop the data collector episode based on grip transition."""
        is_any_active = any(self.active.values())
        was_any_active = any(self._prev_grip_active.values())
        if is_any_active and not was_any_active:
            self.data_collector.start_episode()
            print(f"[DATA] 开始记录 {src_name}")
        elif not is_any_active and was_any_active:
            self.data_collector.end_episode()
            print(f"[DATA] 结束记录 {src_name}")

    def _record_data_step(
        self,
        src_name: str,
        config: dict,
        ...
        """Record one frame of observation and action data."""
        ee_xyz, ee_quat = self._get_link_pose(config["link_name"])
        obs = {
    ...
    def _update_ik(self):
        ...
        if self._debug_xr and time.time() - self._last_debug_t > 2.0:
            self._last_debug_t = time.time()
            for src_name, config in self.manipulator_config.items():
                grip = self.xr_client.get_key_value_by_name(config["control_trigger"])
                pose = self.xr_client.get_pose_by_name(config["pose_source"])
                print(f"[XR] {src_name}: grip={grip:.2f} " f"pose_xyz={[round(p, 3) for p in pose[:3]]}")
            lg = self.xr_client.get_key_value_by_name("left_grip")
            rg = self.xr_client.get_key_value_by_name("right_grip")
            if max(lg, rg) < self._grip_threshold and not self._grip_hint_shown:
                print("[XR] 按住手柄侧边 Grip 键（握把）才能驱动机械臂，" f"当前阈值 {self._grip_threshold:.1f}")
                self._grip_hint_shown = True

        for src_name, config in self.manipulator_config.items():
            xr_grip_val = self.xr_client.get_key_value_by_name(config["control_trigger"])
            self.active[src_name] = xr_grip_val > self._grip_threshold

            # Grip 边界触发数据记录器
            if self.data_collector is not None:
                self._handle_data_collector_grip(src_name)
            # Grip 状态变更钩子（通知子类）: 在更新 _prev_grip_active 之前检查
            ...
            self._prev_grip_active[src_name] = self.active[src_name]
            ...
            if self.active[src_name]:
                ...
                # 记录当前帧数据
                if self.data_collector is not None:
                    self._record_data_step(src_name, config, delta_xyz, delta_rot, xr_grip_val)
            else:
    ...
    def _viz_manipulator_frame(self, name: str, config: dict) -> None:
        """Visualize the target frame and optional motion tracker for a single manipulator."""
